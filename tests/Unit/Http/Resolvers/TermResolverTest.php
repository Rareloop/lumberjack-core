...
class TermResolverTest extends TestCase
{
    ...
    #[Test]
    public function it_throws_an_exception_if_the_queried_object_is_not_a_term(): void
    {
    ...
    #[Test]
    public function it_throws_an_exception_if_the_resolved_term_is_not_of_the_expected_typehinted_class(): void
    {
        $wpTerm = Mockery::mock(WP_Term::class);
        Functions\expect('get_queried_object')->once()->andReturn($wpTerm);

        $timberTerm = Mockery::mock(Term::class); // Not a TermStub
        $timber = Mockery::mock('alias:' . Timber::class);
        $timber->shouldReceive('get_term')->once()->with($wpTerm)->andReturn($timberTerm);
